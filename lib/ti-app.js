...
var jsFiles = ["https://maps.googleapis.com/maps/api/js?key=AIzaSyAvBPL76AXVKbwSpm9X1BlXTzaTXhNLYew", "https://www.gstatic.com/charts/loader.js", "https://www.gstatic.com/firebasejs/3.7.1/firebase.js", "https://www.gstatic.com/firebasejs/3.7.1/firebase-app.js", "https://www.gstatic.com/firebasejs/3.7.1/firebase-auth.js", "https://www.gstatic.com/firebasejs/3.7.1/firebase-database.js"];
...
function setListView(){
	...
	var rivIndex = document.getElementById("listRiver").selectedIndex;
	var stateIndex = document.getElementById("listState").selectedIndex;
	var rivName  = $("#listRiver").val() 
	var stateName  = $("#listState").val() 
		 var rivElm=document.getElementById("listRiver")
		...
		if(stateName==="Any"){
		  ...
		  document.getElementById("listRiver").selectedIndex=rivIndex;
		 document.getElementById("listState").selectedIndex=stateIndex;
	    if(stateName==="Any" && rivName==="Any"){
		    document.getElementById("listContents").innerHTML=""
				  for (var i=0; i<json.rows.length; i++){
						if (isNaN(json.rows[i][5])){
               						var colour = "Khaki !important";
              				 	}
                    				  else if(json.rows[i][5]<json.rows[i][3]){
                     					 var colour = "Tomato !important"
                     				 }
						else if(json.rows[i][5]>json.rows[i][4] && json.rows[i][4]!="-" && json.rows[i][5]!=""){
                    					  var colour = "SkyBlue !important"
                     				 }
                     				 else{
                     					 var colour = "LightGreen !important"
                    				  }
						var node="<div style = \"background-color:"+colour+" !important\"<p>"+json.rows[i][0]+json.rows[i][1]+"</p><p>"+json.rows[i][8]+" kms of grade "+json.rows[i][7]+" rapids; level is "+json.rows[i][5]+"</p></div>"
						var divNode = document.createTextNode(node)
						document.getElementById("listContents").appendChild(divNode);
		    }
	    }
			   else if (stateName!=="Any" && rivName ==="Any"){
				   document.getElementById("listContents").innerHTML=""
				  for (var i=0; i<json.rows.length; i++){
					  if(stateName===json.rows[i][11]){
						if (isNaN(json.rows[i][5])){
						...
						var node="<div style = \"background-color:"+colour+" !important\"<p>"+json.rows[i][0]+json.rows[i][1]+"</p><p>"+json.rows[i][8]+" kms of grade "+json.rows[i][7]+" rapids; level is "+json.rows[i][5]+"</p>"
						var divNode = document.createTextNode(node)
						document.getElementById("listContents").appendChild(divNode)
						  }
				  	}
				...
				document.getElementById("listContents").innerHTML=""
				  for (var i=0; i<json.rows.length; i++){
					  if(stateName===json.rows[i][11] && rivName===json.rows[i][0]){
						if (isNaN(json.rows[i][5])){
               						var colour = "Khaki !important";
              				 	}
                    				  else if(json.rows[i][5]<json.rows[i][3]){
