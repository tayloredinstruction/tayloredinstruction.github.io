...
var jsFiles = ["https://maps.googleapis.com/maps/api/js?key=AIzaSyAvBPL76AXVKbwSpm9X1BlXTzaTXhNLYew", "https://www.gstatic.com/charts/loader.js", "https://www.gstatic.com/firebasejs/3.7.1/firebase.js"];
  ...
  if (snap.val() === true) {
	 ...
	 firebase.auth().onAuthStateChanged(function() {
      ...
      usertriplist();
		       ...
		       document.getElementById("logintrip").className="w3-hide"
		       document.getElementById("loggedInTrip").className=""      
                  var user = firebase.auth().currentUser
		  document.getElementById("userId").value=user.displayName
	  ...
	  firebase.database().ref('trips/active').on('value', function(snapshot) {
	       ...
	       if(snapshot.val()===null || snapshot.val()===undefined || priv===0){
		...
		if(data.private === true || data.space ==="0"){
	      ...
	      setTimeout(function(){localStorage.setItem("offlinePage", document.body.innerHTML);}, 10000);
} else {
	setTimeout(function(){
		connectedRef.once("value").then(function(snapper) {
		if (snapper.val() === true) {
			console.log("Online")
			...
			console.log("Offline")
	var offlinePage = localStorage.getItem("offlinePage");
	if(!(offlinePage===undefined || offlinePage===null)){
...
function setListView(){
	...
	var stateIndex = document.getElementById("listState").selectedIndex;
	var listSort= new Array();
	var stateName  = $("#listState").val()
		 document.getElementById("listState").selectedIndex=stateIndex;
	    if(stateName==="Any"){
		    document.getElementById("listContents").innerHTML=""
				  for (var i=0; i<json.rows.length; i++){
						if (isNaN(parseFloat(json.rows[i][5]))){
               						var colour = "yellow";
              				 	}
					  else{
						    var clev = parseFloat(json.rows[i][5])
						    if (isNaN(parseFloat(json.rows[i][3]))){
							var minlev = json.rows[i][3]
							}
							else{
							  var minlev = parseFloat(json.rows[i][3])
							}
						  if (isNaN(parseFloat(json.rows[i][4]))){
							var maxlev = json.rows[i][4]
							}
							else{
							  var maxlev = parseFloat(json.rows[i][4])
							}
                    				 if(clev<minlev){
                     					 var colour = "red"
                     				 }
						else if(clev>maxlev && maxlev!="-" && clev!=""){
                    					  var colour = "blue"
                     				 }
                     				 else{
                     					 var colour = "green"
						...
						var node="<p>"+json.rows[i][0]+" - "+json.rows[i][1]+"</p><p>"+json.rows[i][8]+" kms of grade "+json.rows[i][7]+" rapids; level is "+json.rows[i][5]+"</p>"
						var divNode = document.createElement('div');
					  	divNode.className="w3-text-black w3-panel w3-pale-"+colour+" w3-leftbar w3-border-"+colour
					  	divNode.innerHTML = node
					  	listSort.push([colour, divNode]);
		    }
	    }
			   else if (stateName!=="Any"){
				   document.getElementById("listContents").innerHTML=""
				  for (var i=0; i<json.rows.length; i++){
					  if(stateName===json.rows[i][11]){
						if (isNaN(parseFloat(json.rows[i][5]))){
               						var colour = "yellow";
              				 	}
					  else{
	...
	var listSort = listSort.sort(listSorter)
	var appendit = document.getElementById("listContents")
	for(var l=0; l<listSort.length; l++){
	appendit.appendChild(listSort[l][1]);	
	}
}
    ...
    function initMap() {
      ...
      var myMapType = new google.maps.ImageMapType({
        getTileUrl: function(coord, zoom) {
          return "http://tile.openweathermap.org/map/precipitation_new/"+zoom+"/"+coord.x+"/"+coord.y+".png?appid=b33c02fe5a68e46356570b59b171a517&dummy="+(new Date()).getHours().toString()+(new Date()).getDate().toString();
        },
      ...
      map.overlayMapTypes.insertAt(0, myMapType);
         ...
         function initialize() {
		 var rivIndex=document.getElementById("mapRiver").selectedIndex;
		 var secIndex=document.getElementById("mapSection").selectedIndex;
		 ...
		 var json = JSON.parse(localStorage.json)
		   var rivName  = $("#mapRiver").val() 
		   var secName  = $("#mapSection").val() 
                   var rivElm=document.getElementById("mapRiver")
		   removeOptions(rivElm);
		...
		for(var k=0;k<checker.length;k++){
			if(json.rows[i][0] === checker[k]){
				count+=1
			}
		}
			   if(count===0){
		checker.push(json.rows[i][0])
                   var div = document.createElement('div');
                   div.innerHTML= json.rows[i][0]
                     var option = document.createElement('option');
                     option.value = json.rows[i][0];
                     option.appendChild(document.createTextNode(div.textContent));
                     df.appendChild(option);
			   }
		   }
		 rivElm.appendChild(df);
		 var secElm=document.getElementById("mapSection")
		 removeOptions(secElm);
               	var df = document.createDocumentFragment();
		var option = document.createElement('option');
                     option.value = "Any";
                     option.appendChild(document.createTextNode("Any"));
		...
		if(rivName==="Any"){
			for (var i=0; i<json.rows.length; i++){
			   ...
			   if(rivName===json.rows[i][0]){
                   ...
                   secElm.appendChild(df);
		  document.getElementById("mapRiver").selectedIndex=rivIndex;
		 document.getElementById("mapSection").selectedIndex=secIndex;
	    if(rivName==="Any"){
		    var rivQuery=""
		    }
			   else{
				   rivQuery=" and col1 \x3d \x27"+rivName+"\x27"
			   }
	 if(secName==="Any"){
		    var secQuery=""
		    }
			   else{
				   secQuery=" and col2 \x3d \x27"+secName+"\x27"
			   }
         ...
         where:  "col15 \x3e\x3d "+miniVal+" and col15 \x3c\x3d "+maxiVal+" and col14 \x3e\x3d "+miniValg+" and col16 \x3c\x3d "+maxiValg+rivQuery+secQuery+" and dummy="+(new Date()).getTime()
             },
             options: {
         styleId: sid,
         templateId: tid
...
function openMTab(event, divName) {
  ...
  for (i = 0; i < x.length; i++) {
    ...
    event.currentTarget.className += " w3-green";
		...
		document.getElementById("loadContainerApp").className=""
		  ...
		  document.getElementById("loadContainerTrip").className=""
		    ...
		    function notificationChecker(){
			    ...
			    if(x===0){
				document.getElementById("tripBadge").innerHTML=""
			    }
				else{
		    ...
		    function approveList(){
		       ...
		       if(snapshot.val().trips===undefined){
			       			       
		       }
		       else{
			for(var i in snapshot.val().trips){
				var item=snapshot.val().trips[i]
				if(item.uid===user){
				var trip=i
				firebase.database().ref('trips/active/'+trip).off()
		...
		firebase.database().ref('users/' + user + '/trips/invited').off()
        ...
        x.className = x.className.replace(" w3-show", "");
            ...
            function usertriplist(){
               ...
               firebase.database().ref('users/' + user).on('value', function(snapshot) {
		         var contents = new Array()
		        if(snapshot.val().trips===undefined){
                        var node = "<p id='notripshere'>No upcoming trips</p><br>"
			para.innerHTML=node
                     }
                        else{
				if($("p#notripshere")){
				$("p#notripshere").remove()
				}
                  for (v in snapshot.val().trips){
			  var childSnapshot=snapshot.val().trips[v]
			  if(v==="invited"){
			  }
			  else{
			firebase.database().ref('trips/active/'+v+'/details/').off
			firebase.database().ref('trips/active/'+v+'/details/').on('value', function(childSnapshot) {
				v=childSnapshot.ref.parent.key
				var data=childSnapshot.val()
					 notificationChecker()
					 if(data===null){
						 if(document.getElementById(v) === null || document.getElementById(v) === undefined){
							 if($("div#mytrips").children().length===0){
							var node = "<p id='notripshere'>No upcoming trips</p><br>"
			para.innerHTML=node	 
							 }
						 } else{
							 
				para.removeChild(document.getElementById(v));
				notificationChecker()
					 }
					 }
					 else{
					...
					var comments = data.comments
						 var head='<div id = '+v+'>'
                     var node = 'Trip Organiser: '+ data.organiser+'<br>Trip Name: ' + data.trip + '<br>Section: ' + data.section + '<br>Date and Time: ' + data.date +'<br>Contact Email: '+ data.email + '<br>Contact Phone Number: ' + data.phone + '<br>Participants: '+data.participants+'<br>Additional Information: ' + data.info + '<div class="w3-accordion"><button onclick="commentShow(\''+v+'comments\')">Comments</button><div id="'+v+'comments" class="w3-accordion-content w3-container"><div id="'+v+'view">'+comments+'</div><input type="text" id="'+v+"text"+'"><br><button onClick="addComment(\''+v+'\')">Send</button></div></div><br>'
			var tail = "</div>"
			if(document.getElementById(v)!==null){
					document.getElementById(v).innerHTML=node;
					}
			else{
		     para.innerHTML+=head+node+tail
			}
    ...
    document.getElementById("favsdiv").innerHTML = "";
	    ...
	    function favMod(element){
			...
			if(div.textContent===element){
				firebase.database().ref('Sections/'+div.textContent).off()
		...
		for(x in favslist){
			...
			var div = document.createElement('div')	
			div.innerHTML=favslist[x]
			if(document.getElementById(div.textContent)===null){
			var cont = document.createElement('div')
			cont.id = div.textContent
			favsdiv.appendChild(cont)
			}
		firebase.database().ref('Sections/'+div.textContent).off()
		firebase.database().ref('Sections/'+div.textContent).on('value', function(snapshot) {
			initialize();
		 ...
		 var blankdiv=false
                      ...
                      else if(data.details.level<data.details.minlevel){
                      var colour = "red !important"
		      		 var blankdiv=true
                      }
			else if(data.details.level>data.details.maxlevel && data.details.maxlevel!=="-"){
			 ...
			 var node = "<div class='w3-bar'><button onclick=\"favMod(this.parentElement.parentElement.id)\" class=\"w3-button w3-right\" style=\"line-height:0;color:red !important;font-size:36px !important\">&times;</button><div id = \""+snapshot.key+" content\"  style = \"border-left: 6px solid "+colour+";\">River: "+data.details.river+" - "+data.details.section+"<br>Levels: "+data.details.minlevel+"-"+data.details.maxlevel+" Current: "+(Math.round(100*parseFloat(data.details.level))/100).toString()+" Previous: "+(Math.round(100*parseFloat(data.details.plevel))/100).toString()+"<br>Conditions: "+data.details.conditions+"<br>Forecast: "+data.details.forecast+"</div></div><div id = \""+snapshot.key+"curve_chart\" style = \"max-width:100% !important; height:"+divHeight+"\"></div><br>";
			document.getElementById(snapshot.key).innerHTML=node
			if(blankdiv){
			var dataArrayHead = [['Date', 'Level']]
			var dataArrayBody = new Array()
			for (var i in data.leveldata){
			//var d = new Date(i)
			dataArrayBody.push([new Date(i),parseFloat(data.leveldata[i])])
...
document.getElementById("logbooklist").selectedIndex = "0";
    ...
    for (var x=0; x<ins.length;x++){
		...
		firebase.database().ref('users/' + user + '/completed/'+trip).on('value', function(snapshot) {
			...
			firebase.database().ref('users/' + user + '/completed/'+logRef+'/details').update(payload);
...
function writeTripInformation() {
   ...
   var json = JSON.parse(localStorage.json)
   var i = document.getElementById('rivsecs').selectedIndex
   if (document.getElementById("rivsecsalt").checked === true){
   ...
   var newtripref = firebase.database().ref().child('users').push().key;
	var payload={
		comments: "",
		other: document.getElementById('rivsecsalt').checked,
	secref: document.getElementById('rivsecs').value,
    ...
    role: "",
    skills: ""}
  firebase.database().ref('users/' + userId.uid + '/trips/' + newtripref).set(payload, function(error){
                  if(error){
   ...
   firebase.database().ref('trips/active/' + newtripref + '/details').set(payload, function(error){
                  if(error){
  ...
  firebase.database().ref('users/' + vals[j].value + '/trips/invited/'+newtripref).set(payload);
   }
		document.getElementById("faddtrip").reset();
        ...
        users.sort(function(a, b) { 
		    ...
		    notificationChecker()
		    updList()
		 document.getElementById("fupdtrip").reset();
            }
	     ...
	     function upIds(trip){
			    ...
			    function updList(){
			var user=firebase.auth().currentUser.uid;
			firebase.database().ref('users/' + user + '/trips/').once('value', function(snapshot) {
			...
			for(i in snapshot.val()){
			   ...
			   function tripgetTrip(){
				 if (document.getElementById("updtriplist").value==="-1"){
					 	document.getElementById("fupdtrip").reset();
				 } else{
		...
		firebase.database().ref('users/' + user + '/trips/'+trip).once('value').then(function(snapshot) {
			var data = snapshot.val()
			if(data.other===true){
			document.getElementById('updrivsecsalttext').value = data.section
				$('#updrivsecsalt').prop('checked', data.other)
			}
			else{
			document.getElementById('updrivsecs').value = data.secref
			}
			$('#updprivate').prop('checked', data.private)
...
function updTripInformation() {
   var userId = firebase.auth().currentUser;
   var json = JSON.parse(localStorage.json)
   var i = document.getElementById("updrivsecs").selectedIndex
   if (document.getElementById("updrivsecsalt").checked === true){
...
var payload={
	comments: "",
	other: document.getElementById('updrivsecsalt').checked,
	secref: document.getElementById('updrivsecs').value, 
...
firebase.database().ref('users/' + upduser + '/trips/' + newtripref).update(payload, function(error){
                  if(error){
                  alert("Trip could not be added" + error)
                  }
                  else{
  ...
  firebase.database().ref('users/' + upduser + '/trips/invited/' + newtripref).update(payload, function(error){
                  if(error){
                  alert("Trip could not be added" + error)
                  }
                  else{
   ...
   firebase.database().ref('trips/active/' + newtripref + '/details').update(payload, function(error){
                  if(error){
  ...
  firebase.database().ref('users/' + vals[j].value + '/trips/invited/'+newtripref).set(payload);
        ...
        users.sort(function(a, b) { 
               ...
               list.appendChild(df)
	     				       		   ...
	     				       		   function openTTab(event, divName) {
  var i, x, tablinks;
    var x = document.getElementsByClassName("trip");
    for (i = 0; i < x.length; i++) {
       x[i].style.display = "none";  
    }
