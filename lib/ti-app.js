    ...
    document.getElementById("favsdiv").innerHTML = "";
		...
		firebase.database().ref('Sections/'+div.textContent).on('value', function(snapshot) {				 
		 ...
		 var blankdiv=false
			 ...
			 var node = "<div class='w3-bar'><button onclick=\"favMod(this.parentElement.parentElement.id)\" class=\"w3-large w3-red w3-button w3-right\">&times;</button><div id = \""+snapshot.key+" content\"  style = \"border-left: 6px solid "+colour+";\">River: "+data.details.river+" - "+data.details.section+"<br>Levels: "+data.details.minlevel+"-"+data.details.maxlevel+" Current: "+(Math.round(100*parseFloat(data.details.level))/100).toString()+" Previous: "+(Math.round(100*parseFloat(data.details.plevel))/100).toString()+"<br>Forecast: "+data.details.forecast+"</div></div><div id = \""+snapshot.key+"curve_chart\" style = \"max-width:100% !important; height:"+divHeight+"\"></div><br>";
